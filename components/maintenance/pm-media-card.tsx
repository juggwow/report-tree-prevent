...
export default function MaintenanceImgMediaCard({
  data,
}: {
  data: ImgMediaCardProp | null;
}) {
    ...
    <Card sx={{ width: 345 }}>
      ...
      <CardActions>
        <Button
          size="small"
          onClick={() =>
            window.open(`https://www.google.com/maps?q=${data.lat},${data.lon}`)
          }
        >
          แผนที่
        </Button>
        <Button
          size="small"
          onClick={() =>
            router.push(`/maintenance/pm-vine/${data.id as string}`)
          }
        >
          {data.maintenance ? "รายละเอียดการแก้ไข" : "แก้ไข"}
        </Button>
      </CardActions>
    </Card>
  );
