...
export default function ChangePlanLVCard({plan,onClickEdit,onClickCancel}:{plan:PlanLVChangeReq,onClickEdit:()=>void,onClickCancel:()=>void}) {
  return (
    <Box >
      ...
      <Divider/>
      <Typography>
        แผนงานเดิม: {plan.oldPlan.peaNo}
      </Typography>
      <Typography variant="body2" gutterBottom>
        <span>ฟีดเดอร์ที่เกาะ: {plan.oldPlan.feeder}</span><br/>
        <span>ความยาว: {plan.oldPlan.distanceCircuit}</span>
      </Typography>
      <Divider/>
      <Typography>
        แผนงานใหม่: {plan.changeReq.peaNo}
      </Typography>
      <Typography variant="body2">
        <span>ฟีดเดอร์ที่เกาะ: {plan.changeReq.feeder}</span><br/>
