...
export default async function handler(
  ...
  if (session && session.sub && session.pea && session.pea.karnfaifa) {
    try {
      ...
      const reportDocs = await planPreventCollection.find(reportfilter).toArray()
      ...
      const resReport = await planPreventCollection.updateMany(reportfilter,reportUpdate)
      ...
      const resEdit = await planPreventCollection.updateMany(editFilter, editUpdate)
