...
export default function ProfilePage({ pea,link }: { pea: peaUser | null,link:string }) {
  ...
  const validateChecker = () => {
    ...
    if(!(peaUser.firstname && /^[\u0E00-\u0E7Fa-zA-Z]+$/.test(peaUser.firstname))){
      return {pass: false, field: "firstname"}
    }

    if(!(peaUser.lastname && /^[\u0E00-\u0E7Fa-zA-Z]+$/.test(peaUser.lastname))){
  ...
  return (
    <div className=" mx-auto mt-3 flex flex-col w-96 shadow-xl rounded-xl bg-white">
      <h3 className=" mx-4 mt-8 text-center"><BorderColorIcon color="inherit"/>{pea?"แก้ไขข้อมูลผู้ใช้":"เพิ่มข้อมูลผู้ใช้"}</h3>
      <form className="mx-4 my-8 flex flex-col" onSubmit={handleSubmit}>
        <Autocomplete
          ...
          onChange={(e, v) =>
            setPeaUser({ ...peaUser, karnfaifa: v?v:"" })
          }
          defaultValue={peaUser?.karnfaifa}
          sx={{ width: "100%" }}
          renderInput={(params) => (
            <TextField required {...params} error={!optionsKarnfaifa.includes(peaUser?.karnfaifa!)} label="สังกัด" />
          )}
        />
        <TextField
          ...
          required
          error={!(peaUser && peaUser.firstname && /^[\u0E00-\u0E7Fa-zA-Z]+$/.test(peaUser.firstname))}
          onChange={(e) => {
            {
        ...
        <TextField
          ...
          required
          error={!(peaUser && peaUser.lastname && /^[\u0E00-\u0E7Fa-zA-Z]+$/.test(peaUser.lastname))}
          onChange={(e) => {
        ...
        <TextField
          ...
          error={!(peaUser && peaUser.userid && /^\d{6,7}$/.test(peaUser.userid))}
          onChange={(e) => {
        ...
        <TextField
          ...
          error={!(peaUser && peaUser.mobileno && /^\d{10}$/.test(peaUser.mobileno))}
          onChange={(e) => {
